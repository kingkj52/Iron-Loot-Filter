 ...
 * mean reimplementing that decision badly and re-checking it after every game update, so nothing
	...
	static boolean canTake(boolean restricted, int ownership, boolean groupDropsTakeable)
	{
		...
		switch (ownership)
		{
			...
			case TileItem.OWNERSHIP_GROUP:
				return groupDropsTakeable;
			default:
				// OWNERSHIP_SELF, plus OWNERSHIP_NONE for world spawns and anything the server
				// never attributed to a player.
				return true;
		}
	}
}
